#Her we compare the time taken for YFP to appear
...
#If it takes longer to appear in old cells, that would suggest that delay in cutting, repair or YFP expression could be one #reason why a lower % of cells is detected as repair.  If it is similar to between old and young, it means that at in cells in #which repair occurs, repair, expression, etc is fast
...
p1 = ggplot(info,aes(expage,lastobservationtime))+geom_boxplot(width=ebwidth)+geom_jitter(height=0,width = jitterw,aes(colour=factor(replabel)))+facet_grid(.~expstrain2,labeller=label_parsed)+geom_hline(aes(yintercept=doxtime+14*6))

ylim(0,40) + ylabel + xlab('age') + geom_hline(yintercept=140/60) + geom_point(data=meanandsem,aes(expage,firstontimeh))+labs(colour = 'Burst?') + themes + border
